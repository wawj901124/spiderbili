...
rootpath = str(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
...
from WWTest.util import HTMLTestRunner as HTMLTestRunner
from WWTest.test.all_test_list_zhongshiyou_pageloadtime import caselist  #调用数组文件
from WWTest.util.getTimeStr import GetTimeStr
from WWTest.util.sendEmail import SendEmail
...
class RunAllTest(unittest.TestCase):
    ...
    def runAllTest(self,testproject=None,testmodule=None):
        ...
        send_e = SendEmail()
        userlist = ['xiang_kaizheng@wanweitech.com']
        send_e.run_send(runresult.success_count, runresult.failure_count,runresult.error_count,filename,userlist=userlist,
...
if __name__ == '__main__':
    runat = RunAllTest()
    # runat.run()
    runat.runAllTest(testproject=u"中石油",testmodule=u"页面加载时间统计")
